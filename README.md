...
This repository is preserved as a historical archive. The measurement files are the primary record. The Python acquisition and plotting utilities are legacy support code; they were checked without laboratory hardware and are not maintained.

Before relying on the dataset, read [KNOWN_ISSUES.md](KNOWN_ISSUES.md), [NOTICE](NOTICE), and [LICENSE](LICENSE). This snapshot makes no new repository-wide license grant.
...
At pre-archive commit `17f0a758`, the repository contained 561 Touchstone files, 303 S11 PNGs, and 227 Smith-chart JPEGs. Archive preparation quarantined one all-NUL Touchstone file and one all-NUL JPEG, and reclassified two identical Touchstone files as test artifacts. The archive contains 558 primary measurements, two readable test files, 303 S11 plots, and 226 readable Smith charts. Plot coverage is incomplete, so absence of a plot does not imply absence of a measurement.
...
The unit token was corrected and trailing option-line whitespace removed. Outside that option line, each readable file is byte-for-byte unchanged. The old header would have made a standards-compliant reader interpret the frequencies one billion times too high. `MANIFEST.csv` records current hashes and, where available, hashes from pre-correction commit `17f0a758`.
...
`requirements-lock.txt` records the Python 3.12 environment captured for
archival checks on 2026-08-16. Use it to repeat those checks.

The validator checks the manifest, Touchstone structure and frequency metadata, image integrity, known quarantined artifacts, and selected cross-file consistency rules. Without full Git history, it skips only the base-commit provenance rebuild. Existing measurements can also be opened with a standards-compliant Touchstone reader such as scikit-rf after installing the dependencies.
...
No `CITATION.cff` is supplied because the responsible creator list and author order have not been confirmed. Until a project-approved citation is available, cite the repository organization, repository title, full commit hash or archival tag, access date, and repository URL. Do not infer individual scientific authorship from commit counts alone.

## Support

This archive promises no ongoing support, new measurements, or hardware compatibility. Direct rights, provenance, or scientific questions to the repository owner.

...
BASE_COMMIT = "17f0a75895d7c08b6a6a7b51cb56b60b8307041c"
HEADERS = [
    "path",
    "sha256",
    "size_bytes",
...
def make_row(
    ...
    if kind == "touchstone_s1p":
            ...
            if corrected_source == data:
                flags.extend(
                    (
                        ...
                        "numeric_rows_preserved_from_17f0a758",
                    )
                )
        if status == "test_artifact":
            ...
            provenance = (
                ...
                "numeric rows preserved from 17f0a758."
            )
        else:
            provenance = (
                "Legacy GHz header corrected to Hz; numeric rows preserved from 17f0a758."
            )
    elif status == "quarantined_corrupt":
        if data and not any(data):
            flags.append("all_NUL_bytes")
        if source_data == data:
            flags.append("bytes_preserved_from_17f0a758")
        ...
        provenance = (
            ...
            "tools/render_s1p_plots.py; replaces the stale pre-retest plot at 17f0a758."
        )
        ...
        if status == "restored_valid":
            ...
            provenance = (
                ...
                "(commit 6689539); replaces the incorrect ANT1445 duplicate at 17f0a758."
            )
        else:
            if source_data == data:
                flags.append("unchanged_from_17f0a758")
            provenance = "Preserved from 17f0a758."
    ...
    return {
        ...
        "source_sha256_at_17f0a758": source_digest,
